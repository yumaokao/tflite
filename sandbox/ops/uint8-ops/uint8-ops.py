...
def main(args, op):
  ...
  if args.mnist:
    ...
    batch_xs = (np.random.rand(10, 784) - 0.25) * 4
    batch_xs = batch_xs.astype('float32')

  # Run test
  ys = sess.run(plugin.model_tf(y), feed_dict={x: batch_xs})
  batch_xs2 = sess.run(plugin.cvt_x(x), feed_dict={x: batch_xs})

  # Save to npy
  np.save(os.path.join(exportbase, 'batch_xs.npy'), batch_xs2.output)
  np.save(os.path.join(exportbase, 'ys.npy'), ys.output)
  batch_xs2.output.tofile(os.path.join(exportbase, 'batch_xs.bin'))
  ys.output.tofile(os.path.join(exportbase, 'ys.bin'))

  # Save weights to npy
  for k in variables:
    v = variables[k]
    np.save(os.path.join(exportbase, '{}.npy'.format(k)), v.eval())
    v.eval().tofile(os.path.join(exportbase, '{}.bin'.format(k)))
