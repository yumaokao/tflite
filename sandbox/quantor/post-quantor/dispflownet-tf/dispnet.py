...
def input_pipeline(filenames, input_size, batch_size, num_epochs=None, shuffle=True):
    if shuffle is True:
        ...
        filename_queue = tf.train.input_producer(
            filenames, element_shape=[3], num_epochs=num_epochs, shuffle=False)
    ...
    if shuffle is True:
        ...
        left_img_batch, right_img_batch, target_batch = tf.train.batch(
            [left_img, right_img, target], batch_size=batch_size)
...
def build_main_graph(left_image_batch, right_image_batch, is_corr=True):
    if is_corr:
...
class DispNet(object):
    def __init__(self, mode="inference", ckpt_path=".", dataset=None,
                 input_size=INPUT_SIZE, batch_size=4, is_corr=True):
